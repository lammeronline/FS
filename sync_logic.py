...
def ensure_path_is_ready(path_str, net_creds=None):
    ...
    try:
        if sys.platform == "win32":
            ...
            subprocess.run(command, capture_output=True, text=True, check=True, encoding='cp866')
        ...
        logging.error(f"Не удалось подключить сетевой диск '{path_str}': {e}")
        return False
...
def get_files_map(directory, exclude_patterns=None, stop_event=None, comparison_mode='accurate', use_parallel=False):
    ...
    for file_path in root_path.rglob('*'):
        if stop_event and stop_event.is_set():
            raise SyncCancelledError("Сканирование прервано.")
    ...
    def process_file(file_info):
        ...
        if comparison_mode == 'hybrid':
            return file_info['rel_path'], (file_info['size'], file_info['mtime'], None)
    ...
    for result in results:
        if result:
            ...
            files_map[rel_path] = data
            
    return files_map

def sync_folders(source_dir, dest_dir, no_overwrite, delete_removed, sync_empty_dirs=False, exclude_patterns=None, stop_event=None, comparison_mode='accurate', use_parallel=False):
    ...
    for rel_path, source_data in source_files.items():
        ...
        if rel_path not in dest_files:
            needs_update = True
            reason = "КОПИРОВАНИЕ (новый)"
        else:
            dest_data = dest_files[rel_path]
            if comparison_mode == 'hybrid':
                source_size, source_mtime, _ = source_data
                dest_size, dest_mtime, _ = dest_data
                if source_size != dest_size or int(source_mtime) != int(dest_mtime):
                    ...
                    if source_hash != dest_hash:
                        needs_update = True
                        reason = "ОБНОВЛЕНИЕ (изменен)"
            elif source_data != dest_data:
                needs_update = True
                reason = "ОБНОВЛЕНИЕ (изменен)"
        
        if needs_update:
            if no_overwrite and rel_path in dest_files:
                ...
                try:
                    ...
                    if rel_path in dest_files: stats["updated"] += 1
                    else: stats["copied"] += 1
                except (IOError, PermissionError) as e:
                    logging.error(f"Ошибка операции с файлом {rel_path}: {e}")
                    stats["errors"] += 1
    ...
    if delete_removed:
        for rel_path in dest_files:
            ...
            if rel_path not in source_files:
                ...
                except (IOError, PermissionError) as e: logging.error(f"Ошибка удаления файла {rel_path}: {e}"); stats["errors"] += 1
...
def run_sync_session(source, destination, no_overwrite, delete_removed, sync_empty_dirs=False, exclude_patterns=None, source_creds=None, dest_creds=None, stop_event=None, comparison_mode='accurate', use_parallel=False):
    ...
    logging.info(f"Режим сравнения: {comparison_mode}"); logging.info(f"Параллельное сканирование: {'Да' if use_parallel else 'Нет'}"); logging.info("="*50)
    try:
        ...
        stats = sync_folders(source, destination, no_overwrite, delete_removed, sync_empty_dirs, exclude_patterns, stop_event, comparison_mode, use_parallel)
        duration = datetime.now() - start_time
        summary = (f"✅ *Синхронизация успешно завершена!*\n\n*Источник:* `{source}`\n*Назначение:* `{destination}`\n"
                   f"Время выполнения: `{duration}`\n\n*Статистика:*\n- Скопировано новых: *{stats['copied']}*\n- Обновлено: *{stats['updated']}*\n"
                   f"- Пропущено: *{stats['skipped']}*\n- Удалено: *{stats['deleted']}*\n- Создано директорий: *{stats.get('dirs_created', 0)}*\n"
