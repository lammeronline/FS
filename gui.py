...
from tkinter import ttk # Для разделителя
...
# ... (Классы QueueHandler, SettingsWindow, AboutWindow без изменений) ...
...
class SettingsWindow(Toplevel):
    def __init__(self, master):
        ...
        self.transient(master); self.title("Настройки"); self.geometry("400x150"); self.resizable(False, False); self.grab_set()
...
class AboutWindow(Toplevel):
    def __init__(self, master):
        super().__init__(master)
        self.transient(master); self.title("О программе"); self.geometry("350x200"); self.resizable(False, False); self.grab_set()
        ...
        tk.Label(main_frame, text="Версия 1.8").grid(row=1, column=1, sticky="w"); tk.Label(main_frame, text="Утилита для синхронизации файлов.").grid(row=2, column=1, sticky="w", pady=(5,0))
...
class SyncApp:
    ...
    def create_widgets(self):
        ...
        # ... (остальные фреймы и виджеты без изменений)
    ...
    def toggle_source_creds(self):
        if self.source_is_network_var.get():
            self.source_user_label.grid(row=1, column=0, sticky="e", padx=(10,0)); self.source_user_entry.grid(row=1, column=1, sticky="ew", padx=5)
            self.source_pass_label.grid(row=2, column=0, sticky="e", padx=(10,0)); self.source_pass_entry.grid(row=2, column=1, sticky="ew", padx=5)
            self.source_show_pass_check.grid(row=2, column=2, sticky="w")
        else:
            self.source_user_label.grid_forget(); self.source_user_entry.grid_forget(); self.source_pass_label.grid_forget(); self.source_pass_entry.grid_forget(); self.source_show_pass_check.grid_forget()
            self.source_user_var.set(""); self.source_pass_var.set("")
        self.update_save_pass_visibility()

    def toggle_dest_creds(self):
        if self.dest_is_network_var.get():
            self.dest_user_label.grid(row=6, column=0, sticky="e", padx=(10,0)); self.dest_user_entry.grid(row=6, column=1, sticky="ew", padx=5)
            self.dest_pass_label.grid(row=7, column=0, sticky="e", padx=(10,0)); self.dest_pass_entry.grid(row=7, column=1, sticky="ew", padx=5)
            self.dest_show_pass_check.grid(row=7, column=2, sticky="w")
        else:
            self.dest_user_label.grid_forget(); self.dest_user_entry.grid_forget(); self.dest_pass_label.grid_forget(); self.dest_pass_entry.grid_forget(); self.dest_show_pass_check.grid_forget()
            self.dest_user_var.set(""); self.dest_pass_var.set("")
        self.update_save_pass_visibility()
    ...
    def _load_state(self):
        ...
        if os.path.exists(APP_STATE_FILE):
            ...
            self.source_is_network_var.set(config.getboolean('State', 'source_is_network', fallback=False))
            self.dest_is_network_var.set(config.getboolean('State', 'dest_is_network', fallback=False))
    ...
    def start_sync_thread(self):
        ...
        if self.source_is_network_var.get() and self.source_user_var.get() and self.source_pass_var.get():
            source_creds = {'user': self.source_user_var.get(), 'password': self.source_pass_var.get()}
        if self.dest_is_network_var.get() and self.dest_user_var.get() and self.dest_pass_var.get():
    ...
    def setup_context_menus(self):
        ...
        for widget in widgets_with_menu: widget.bind("<Button-3><ButtonRelease-3>", context_menu.popup)
    ...
    def create_menu(self):
        ...
        file_menu = tk.Menu(menubar, tearoff=0); file_menu.add_command(label="Настройки", command=self.open_settings); file_menu.add_separator(); file_menu.add_command(label="Выход", command=self.master.quit)
