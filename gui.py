...
class SyncApp:
    ...
    def import_job_file(self):
        ...
        try:
            ...
            self.source_var.set(job.get('source', ''))
            self.dest_var.set(job.get('destination', ''))
            self.no_overwrite_var.set(job.getboolean('no_overwrite', False))
            self.delete_removed_var.set(job.getboolean('delete_removed', False))
            self.sync_empty_dirs_var.set(job.getboolean('sync_empty_dirs', False))
            self.exclude_patterns_var.set(job.get('exclude', ''))
            self.use_staging_var.set(job.getboolean('use_staging', fallback=True))
            self.use_trash_var.set(job.getboolean('use_trash', fallback=True))
            if 'SourceNetCreds' in job_config:
                self.source_is_network_var.set(True)
                s_creds = job_config['SourceNetCreds']
                self.source_user_var.set(s_creds.get('user', ''))
                self.source_pass_var.set(s_creds.get('password', ''))
            else: self.source_is_network_var.set(False)
            if 'DestNetCreds' in job_config:
                self.dest_is_network_var.set(True)
                d_creds = job_config['DestNetCreds']
                self.dest_user_var.set(d_creds.get('user', ''))
                self.dest_pass_var.set(d_creds.get('password', ''))
            else: self.dest_is_network_var.set(False)
            self.toggle_source_creds()
            self.toggle_dest_creds()
    ...
    def export_job_file(self):
        ...
        if not filepath: return
        job_config = configparser.ConfigParser()
        job_config.add_section('SyncJob')
        job_config.set('SyncJob', 'source', self.source_var.get())
        job_config.set('SyncJob', 'destination', self.dest_var.get())
        job_config.set('SyncJob', 'no_overwrite', str(self.no_overwrite_var.get()).lower())
        job_config.set('SyncJob', 'delete_removed', str(self.delete_removed_var.get()).lower())
        job_config.set('SyncJob', 'sync_empty_dirs', str(self.sync_empty_dirs_var.get()).lower())
        job_config.set('SyncJob', 'exclude', self.exclude_patterns_var.get())
        job_config.set('SyncJob', 'use_staging', str(self.use_staging_var.get()).lower())
        job_config.set('SyncJob', 'use_trash', str(self.use_trash_var.get()).lower())
        config = configparser.ConfigParser()
        config.read(sync_logic.CONFIG_FILE)
        job_config.set('SyncJob', 'comparison_mode', config.get('performance', 'comparison_mode', fallback='accurate'))
        job_config.set('SyncJob', 'use_parallel', config.get('performance', 'use_parallel', fallback='false'))
        if self.source_is_network_var.get() and self.source_user_var.get():
            job_config.add_section('SourceNetCreds')
            job_config.set('SourceNetCreds', 'user', self.source_user_var.get())
            job_config.set('SourceNetCreds', 'password', self.source_pass_var.get())
        if self.dest_is_network_var.get() and self.dest_user_var.get():
            job_config.add_section('DestNetCreds')
            job_config.set('DestNetCreds', 'user', self.dest_user_var.get())
            job_config.set('DestNetCreds', 'password', self.dest_pass_var.get())
        try:
            with open(filepath, 'w', encoding='utf-8') as configfile:
                job_config.write(configfile)
            messagebox.showinfo("Успешно", f"Задача успешно экспортирована в:\n{filepath}")
        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось экспортировать файл задачи:\n{e}")
    ...
    def toggle_source_creds(self):
        if self.source_is_network_var.get():
            ...
            self.source_show_pass_check.grid(row=2, column=2, sticky="w")
        else:
            ...
            self.source_show_pass_check.grid_forget()
    ...
    def toggle_dest_creds(self):
        if self.dest_is_network_var.get():
            self.dest_user_label.grid(row=6, column=0, sticky="e", padx=(10,0))
            self.dest_user_entry.grid(row=6, column=1, sticky="ew", padx=5)
            self.dest_pass_label.grid(row=7, column=0, sticky="e", padx=(10,0))
            self.dest_pass_entry.grid(row=7, column=1, sticky="ew", padx=5)
            self.dest_show_pass_check.grid(row=7, column=2, sticky="w")
        else:
            ...
            self.dest_show_pass_check.grid_forget()
            self.dest_user_var.set("")
            self.dest_pass_var.set("")
        self.update_save_pass_visibility()

    def update_save_pass_visibility(self):
        if self.source_is_network_var.get() or self.dest_is_network_var.get():
            self.save_pass_check.grid(row=8, columnspan=4, sticky='w', pady=(10,0))
        else:
            self.save_pass_check.grid_forget()
            self.save_passwords_var.set(False)

    def toggle_source_pass_visibility(self):
        self.source_pass_entry.config(show="" if self.source_show_pass_var.get() else "*")

    def toggle_dest_pass_visibility(self):
        self.dest_pass_entry.config(show="" if self.dest_show_pass_var.get() else "*")
    ...
    def _load_state(self):
        ...
        if os.path.exists(APP_STATE_FILE):
            ...
            if 'State' in config:
                state = config['State']
                self.source_var.set(state.get('last_source', fallback=''))
                self.dest_var.set(state.get('last_destination', fallback=''))
                self.exclude_patterns_var.set(state.get('last_exclusions', fallback='*.tmp, *.log'))
                self.sync_empty_dirs_var.set(state.getboolean('sync_empty_dirs', fallback=False))
                self.source_is_network_var.set(state.getboolean('source_is_network', fallback=False))
                self.dest_is_network_var.set(state.getboolean('dest_is_network', fallback=False))
                self.save_passwords_var.set(state.getboolean('save_passwords', fallback=False))
                self.use_staging_var.set(state.getboolean('use_staging', fallback=True))
                self.use_trash_var.set(state.getboolean('use_trash', fallback=True))
                if self.save_passwords_var.get():
                    try:
                        self.source_user_var.set(base64.b64decode(state.get('s_user', '')).decode('utf-8'))
                        self.source_pass_var.set(base64.b64decode(state.get('s_pass', '')).decode('utf-8'))
                        self.dest_user_var.set(base64.b64decode(state.get('d_user', '')).decode('utf-8'))
                        self.dest_pass_var.set(base64.b64decode(state.get('d_pass', '')).decode('utf-8'))
                    except Exception:
                        pass
        self.toggle_source_creds()
        self.toggle_dest_creds()
    ...
    def _save_state(self):
        ...
        config['State'] = {
            ...
            'save_passwords': str(self.save_passwords_var.get()), 'use_staging': str(self.use_staging_var.get()), 'use_trash': str(self.use_trash_var.get())
        }
        if self.save_passwords_var.get():
            state = config['State']
            state['s_user'] = base64.b64encode(self.source_user_var.get().encode('utf-8')).decode('utf-8')
            state['s_pass'] = base64.b64encode(self.source_pass_var.get().encode('utf-8')).decode('utf-8')
            state['d_user'] = base64.b64encode(self.dest_user_var.get().encode('utf-8')).decode('utf-8')
            state['d_pass'] = base64.b64encode(self.dest_pass_var.get().encode('utf-8')).decode('utf-8')
        with open(APP_STATE_FILE, 'w', encoding='utf-8') as configfile:
            config.write(configfile)

    def update_progress(self, p_type, current, total, message=""):
        if p_type == 'overall':
            self.status_text_var.set(message)
            self.overall_progress['value'] = (current / total) * 100 if total > 0 else 0
        elif p_type == 'reset':
            self.status_text_var.set(message or "")
    ...
    def start_sync_thread(self):
        ...
        thread_args = (
            source, dest, self.no_overwrite_var.get(), self.delete_removed_var.get(), self.sync_empty_dirs_var.get(),
            exclude_list, source_creds, dest_creds, self.stop_event, comparison_mode, use_parallel,
    ...
    def run_sync_task(self, source, dest, no_overwrite, delete_removed, sync_empty_dirs, exclude_patterns, source_creds, dest_creds, stop_event, comparison_mode, use_parallel, use_staging, use_trash, progress_callback):
        try:
            ...
            self.log_queue.put(('progress', ('reset', 0, 0, 'Готово!')))
        except sync_logic.SyncCancelledError as e:
            self.log_queue.put(('progress', ('reset', 0, 0, 'Прервано')))
            logging.warning(f"Процесс синхронизации был корректно остановлен: {e}")
        except Exception as e:
            self.log_queue.put(('progress', ('reset', 0, 0, 'Ошибка!')))
    ...
    def browse_source(self):
        self.source_var.set(filedialog.askdirectory() or self.source_var.get())
    
    def browse_dest(self):
        self.dest_var.set(filedialog.askdirectory() or self.dest_var.get())
