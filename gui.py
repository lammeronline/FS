...
# ... (Классы QueueHandler, ContextMenu, SettingsWindow, AboutWindow остаются без изменений) ...
...
class ContextMenu:
    def __init__(self, master):
        self.menu = tk.Menu(master, tearoff=0); self.menu.add_command(label="Вырезать", command=self.cut); self.menu.add_command(label="Копировать", command=self.copy); self.menu.add_command(label="Вставить", command=self.paste); self.widget = None
...
class SyncApp:
    ...
    def create_menu(self):
        ...
        file_menu.add_command(label="Сохранить задачу...", command=self.save_job_file) # <-- НОВЫЙ ПУНКТ
        file_menu.add_separator()
        file_menu.add_command(label="Настройки Telegram", command=self.open_settings)
        file_menu.add_separator()
    ...
    def save_job_file(self):
        """Собирает текущие настройки из GUI и сохраняет их в .ini файл для CLI."""
        filepath = filedialog.asksaveasfilename(
            defaultextension=".ini",
            filetypes=[("Job Files", "*.ini"), ("All Files", "*.*")],
            title="Сохранить файл задачи"
        )
        if not filepath: # Пользователь отменил сохранение
            return

        job_config = configparser.ConfigParser()
        job_config.add_section('SyncJob')
        
        # Собираем основные настройки
    ...
    def setup_context_menus(self):
        context_menu = ContextMenu(self.master); widgets_with_menu = [self.source_entry, self.dest_entry, self.source_user_entry, self.source_pass_entry, self.dest_user_entry, self.dest_pass_entry, self.exclude_entry]
