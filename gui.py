...
class SyncApp:
    ...
    def create_context_menu(self):
        ...
        menu.add_command(label="Вырезать", command=lambda: self.master.event_generate("<<Cut>>"))
        menu.add_command(label="Копировать", command=lambda: self.master.event_generate("<<Copy>>"))
        menu.add_command(label="Вставить", command=lambda: self.master.event_generate("<<Paste>>"))
        ...
        self.master.bind_class("Entry", "<Button-3>", show_menu)
        self.master.bind_class("Entry", "<Button-2>", show_menu) # For macOS
    ...
    def _load_state(self):
        ...
        if os.path.exists(APP_STATE_FILE):
            ...
            if 'State' in config:
                ...
                self.use_staging_var.set(state.getboolean('use_staging', fallback=True)); self.use_trash_var.set(state.getboolean('use_trash', fallback=True))
    ...
    def update_progress(self, p_type, current, total, message=""):
        if p_type == 'overall': self.status_text_var.set(message); self.overall_progress['value'] = (current / total) * 100 if total > 0 else 0
        elif p_type == 'reset': self.status_text_var.set(message or ""); self.overall_progress['value'] = 0
    ...
    def on_closing(self):
        if self.stop_event and not self.stop_event.is_set():
            if messagebox.askyesno("Подтверждение", "Синхронизация еще не завершена. Вы уверены, что хотите прервать процесс и выйти?"):
    ...
    def poll_log_queue(self):
        while True:
            try:
                ...
                if isinstance(message, tuple) and message[0] == 'progress': _, args = message; self.update_progress(*args)
                else: self.display_log(message)
...
if __name__ == "__main__":
    ...
    try:
        ...
        messagebox.showwarning("Ресурс не найден", "Файл иконки 'assets/icon.png' не найден. Программа будет использовать стандартную иконку.")
    except Exception as e:
